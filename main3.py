...
def add_new_animal(db):
   ...
   'Chevrotains are able to hold their breath for about four minutes.',
    ...
    for item in ALL_FACTS:
      ...
      data = {}
      for fact in item:
        data.update({f"fact{fact_counter}": fact})
        fact_counter += 1
  ...
  #   return
    ...
    # db.collection("animal_facts").document(name).set(data)
    # db.collection("animal_facts").document(name).set(data2)
...
#     name = db.collection("animal_facts").document(random).get()
#     fact = db.collection("animal_facts").documnet(name)
...
#     # Save this in the log collection in Firestore
#     log_transaction(db, f"Used {name} {data}")
